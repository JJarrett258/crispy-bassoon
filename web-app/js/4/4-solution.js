...
const extractFieldsFromPassport = {
    byr: (passport) => /(byr):(\d{4})/g.exec(passport),
    iyr: (passport) => /(iyr):(\d{4})/g.exec(passport),
    eyr: (passport) => /(eyr):(\d{4})/g.exec(passport),
    hgt: (passport) => /(hgt):([0-9]+)([a-z]{2})/g.exec(passport),
    hcl: (passport) => /(hcl):#([0-9, a-f]{6})/g.exec(passport),
    ecl: (passport) => /(ecl):(amb|blu|brn|gry|grn|hzl|oth)/g.exec(passport),
    pid: (passport) => /(pid):(\d{9})/g.exec(passport)
};
...
// todo: DONE
export const dateRangeChecks = (byr_iyr_eyr) => {
...
//todo: DONE
export const heightChecker = (hgt) => {
...
//todo: DONE
export const remainingChecks = (passport) => {
...
const validatePassportData = (passportData) => {
    let sortedPassportData = passportData.split(/\n\n/g);
